...
const getPlayerStat = (data,callback,errorCallback) => {
    http.get('/spring/info/player?num=' + data, header())
        .then(res => {
            // RestAPI 서버가 null 을 응답하는 경우
            if(res == null) {
                let error = {msg : 'Server response is NULL'};
        ...
        .catch(err => {
            err.msg = 'getPlayerStat Exception Occurred';
            errorCallback(err);
        });
}
...
const PlayerAPI = {
    ...
    getPlayerStat:(data,callback, errorCallback)=>getPlayerStat(data,callback, errorCallback),
}
